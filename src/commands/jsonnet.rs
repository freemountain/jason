...
fn eval<'b>(matches: &ArgMatches<'b>) -> Result<(), Box<StdError>> {
    ...
    let output = if matches.is_present("exec") {
        ...
        vm.evaluate_snippet("INPUT", expr).unwrap()
    } else {
        let file = matches.value_of_os("INPUT").unwrap();
        vm.evaluate_file(file).unwrap()
    };
...
fn fmt<'b>(matches: &ArgMatches<'b>) -> Result<(), Box<StdError>> {
    let mut vm = JsonnetVm::new();
    ...
    let output = if matches.is_present("exec") {
        let expr = matches.value_of("INPUT").unwrap();
        vm.fmt_snippet("INPUT", expr).unwrap()
    } else {
        let file = matches.value_of_os("INPUT").unwrap();
        vm.fmt_file(file).unwrap()
    };
...
pub fn run<'a>(matches: &ArgMatches) -> CommandResult {
    //fn main() {
